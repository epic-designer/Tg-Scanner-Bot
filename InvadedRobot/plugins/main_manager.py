...
async def formatting(_, message):
    await message.reply_photo(""https://telegra.ph/file/06a754526911af5baecdf.jpg, strings.FORMAT_TEXT)
...
async def scan(_, message):
      ...
      if rank == "Civilian":
          return await message.reply_photo(""https://telegra.ph/file/06a754526911af5baecdf.jpg, "`You Don't Have Enough Rights To Scan...`")
      elif len(message.command) <2:
          return await message.reply_photo(""https://telegra.ph/file/06a754526911af5baecdf.jpg, "`Get Format For Scan By Sending` `/formatting`")
      elif reply and rank == "Troop":
        try:
            msg = await message.reply_photo(""https://telegra.ph/file/06a754526911af5baecdf.jpg, "`Requesting to Invaded...`")
            ...
            if (await is_scan_user(user_id)) == True:
                 await msg.edit_caption("`The user already scanned in Invaded no need request.`")
            else:
...
InlineKeyboardButton("Disapprove Scan",callback_data="disapprove_scan:{message.from_user.id}:{user_id}")]]))
                await msg.edit_caption("the user Successfully requested to Invaded")
...
InlineKeyboardButton("Disapprove Scan",callback_data="disapprove_scan:{message.from_user.id}:{user_id}")]]))
         ...
         try:
            ...
            mention = f"[{user_id}](tg://user?id={user_id})"
            msg = await message.reply_photo(""https://telegra.ph/file/06a754526911af5baecdf.jpg, "`Scanning...`")
            if (await is_scan_user(user_id)) == True:
                  ...
                  await msg.edit_caption("`The Following User Was Already Scanned`\n`So I Have Just Updated The New Details!`")
            else:
                  await add_scan_user(user_id,reason,date)
                  await bot.send_message(config.LOG_CHANNEL_ID, text=strings.SCAN_TEXT.format(message.from_user.id,mention,reason,date))
                  await msg.edit_caption("`The Following User Was Successfully Scanned!`")
         except Exception as e:
             await msg.delete()
...
async def revert(_, message):
      ...
      msg = await message.reply_photo(""https://telegra.ph/file/06a754526911af5baecdf.jpg, "`Reverting Scan...`")
      if not user_id in (await RANK_USERS()):
          return await msg.edit_caption("`You Don't Have Enough Rights To Scan...`")
      elif reply:
           try:
              user_id = int(reply.from_user.id)
              if (await is_scan_user(user_id)) == False:
                  return await msg.edit_caption("`The Following User Is Not A Scanned User How Can I Revert A User's Scan Who Wasn't Scanned ?`")
              else:
                  await remove_scan_user(user_id)
                  await msg.edit_caption("`Successfully Reverted A Scan Of The User...`") 
                  #bot.send()
           except Exception as e:
                await msg.delete()
                await message.reply_photo("https://telegra.ph/file/f21e5445b3d0897f63f3d.jpg", caption=f"`{e}`")
